...
| Export everything | `GET /api/v1/links?limit=1000` |
| Health | `GET /healthz` |

## Agent workflow

1. **Check health:** `GET /healthz` (HTTP 200 = available).
2. **Authenticate:** if the server requires it, send
   `Authorization: Bearer $LINKSERVER_ADMIN_TOKEN` on every `/api/v1/*` call.
   Redirect endpoints (`GET /{path}`) are public.
3. **Create** with the smallest body that satisfies the task; omit `path` to get
   an auto-generated short code.
4. **Parse JSON** responses; every link object includes `short_url`, `status`,
...
| Field | Type | Notes |
|-------|------|-------|
| `target_url` | string (required) | Must be `http`/`https`; private/internal hosts are rejected. |
| `path` | string | Custom path; supports multiple segments (`a/b/c`). Omit to auto-generate. |
...
A wrong/missing password returns `401`. Expired or exhausted links return `410`.
Disabled or unknown links return `404`. Any query string on the visit is merged
into the target URL.
